...
// Icons
import { Users, MessageSquare, Ticket as TicketIcon, Loader2, Plus } from "lucide-react";
...
import type { WebSocketNotification, WebSocketNewMessage, WebSocketChatUpdate } from "@/contexts/WebSocketContext";
import * as crmAgentsService from "@/services/crmAgentsService";
import * as crmChatsService from "@/services/crmChatsService";
import { playNotificationSound, enableAudioNotifications } from "@/utils/audioNotification";
 ...
 */
export const CustomerService = ({ filterType, onFilterChange }: CustomerServiceProps) => {
  // ==========================================================================
  // AUTH & USER
  // ==========================================================================
  ...
  // --------------------------------------------------------------------------
  const { wsStatus, reconnectAttempts, subscribeToMessages, resetUnreadCount } = useWebSocket();
    ...
    const fetchChats = async () => {
      try {
        ...
        const apiChats = await crmChatsService.getChats({
          ...
          channel: filters.channel !== "all" ? (filters.channel as any) : undefined,
        ...
        const transformedChats: Chat[] = await Promise.all(
          apiChats?.chats?.map(async (apiChat) => {
            ...
            return {
              ...
              assignedTo: (apiChat.handled_by === 'human'
              ...
              status: apiChat.status as "open" | "pending" | "assigned" | "resolved",
              ...
              solvedBy: apiChat.resolved_by_agent_id ? assignedAgent?.name : undefined,
  ...
  useEffect(() => {
    ...
    const fetchChatDetails = async () => {
      try {
        ...
        const messagesResponse = await crmChatsService.getChatMessages(activeChat);

        // Transform messages to frontend format
        // Uses sender_name directly from API response
        const transformedMessages: Message[] = messagesResponse.messages.map((apiMsg) => ({
          id: apiMsg.id,
   ...
   */
  const handleNewMessageNotification = useCallback((notification: WebSocketNewMessage) => {
    ...
    //   console.log('⏭️ Message already processed, skipping:', message_id);
    //   return;
    // }
    ...
    setChats((prevChats) => {
      ...
      updatedChats[chatIndex] = {
        ...
        unreadCount: chat.id === activeChat ? chat.unreadCount : chat.unreadCount + 1,
      };
  ...
  }, [activeChat]); // activeChat is the only external dependency

  /**
   * Handle chat update notification (assignment, escalation, status change)
   * Memoized to prevent unnecessary WebSocket reconnections
   */
  const handleChatUpdateNotification = useCallback((notification: WebSocketChatUpdate) => {
    ...
    setChats((prevChats) =>
      prevChats.map((chat) => {
        ...
        if (update_type === 'escalated') {
          ...
          toast.info(`Chat escalated: ${data.reason || 'No reason provided'}`);
        }
  ...
  const handleWebSocketMessage = useCallback((notification: WebSocketNotification) => {
    console.log('📩 WebSocket notification received:', notification.type, notification);
  ...
  }, [handleNewMessageNotification, handleChatUpdateNotification]); // Depend on memoized handlers
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      if (selectedChat.humanId && selectedChat.humanId !== user.id) {
        toast.error('Hanya agent yang ditugaskan yang dapat mengirim pesan ke chat ini');
        return;
      }
      ...
      const transformedMessage: Message = {
        id: sentMessage.id,
        sender: "agent",
        senderName: selectedChat.isAssigned ? selectedChat.assignedTo! : "AI Assistant",
        ...
        prevChats.map((chat) =>
          chat.id === activeChat
            ? {
                ...
                assignedTo: updatedChat.human_agent_name || updatedChat.ai_agent_name || "You",
                humanAgentId: updatedChat.human_agent_id,
                humanAgentName: updatedChat.human_agent_name,
                handledBy: updatedChat.handled_by,
                status: updatedChat.status as "open" | "pending" | "assigned" | "resolved",
              }
            : chat
        )
      );
  ...
  const handleEscalateChat = async (humanAgentId: string, reason?: string) => {
    ...
    try {
      ...
      const errorMessage = error?.response?.data?.detail || 'Gagal escalate chat';
      toast.error(errorMessage);
    }
  };

  const handleAddAgent = async (agentData: Omit<Agent, "id" | "assignedChats" | "resolvedToday" | "avgResponseTime" | "lastActive">) => {
    try {
  ...
  const handleUpdateAgentStatus = async (agentId: string, status: "active" | "inactive" | "busy") => {
    try {
      const updatedAgent = await crmAgentsService.updateAgentStatus(agentId, status as AgentStatus);
      setAgents((prev) =>
        prev.map((agent) =>
  ...
  const handleCreateTicket = async (ticket: Omit<Ticket, "id" | "ticketNumber" | "createdAt" | "updatedAt" | "relatedMessages">) => {
    if (!activeChat) return;
    ...
    try {
      ...
      const newTicket: Ticket = {
        ...
        priority: createdTicket.priority as "low" | "medium" | "high" | "urgent",
        status: createdTicket.status as "open" | "in_progress" | "resolved" | "closed",
  ...
  const handleCreateChat = async (data: {
    ...
    try {
      ...
      const newChat: Chat = {
        ...
        channel: data.channel || '-',
        status: data.assignedAgentId ? "assigned" : (createdChat.status as "open" | "pending" | "assigned" | "resolved"),
        messages: [],
        labels: [],
  ...
  const handleUpdateTicket = async (ticketId: string, updates: Partial<Ticket>) => {
    try {
      ...
      const updatedTicket = await crmChatsService.updateTicket(ticketId, apiUpdates);
          ...
          tickets: (chat.tickets || []).map((ticket) =>
            ticket.id === ticketId
              ? {
                  ...ticket,
                  ...updates,
                  updatedAt: new Date(updatedTicket.updated_at).toLocaleString(),
  ...
  return (
    ...
    >
      ...
      <div className="border-b bg-card p-2">
        <Tabs value={viewMode} onValueChange={(value) => setViewMode(value as "chat" | "ticketing")}>
          <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 h-9 p-1">
            <TabsTrigger value="chat" className="flex items-center justify-center gap-1.5 text-xs data-[state=active]:bg-background">
              <MessageSquare className="h-3.5 w-3.5" />
              Chat View
            </TabsTrigger>
            <TabsTrigger value="ticketing" className="flex items-center justify-center gap-1.5 text-xs data-[state=active]:bg-background">
          ...
          <>
            ...
            <div className="w-64 flex flex-col border-r">
              ...
              <div className="p-2 border-b bg-card space-y-2 flex-shrink-0">
                ...
                <Tabs value={filterType} onValueChange={(value) => onFilterChange(value as "assigned" | "unassigned")}>
                  <TabsList className="grid w-full grid-cols-2 h-9 p-1">
                    <TabsTrigger value="unassigned" className="text-xs data-[state=active]:bg-background">Unassigned</TabsTrigger>
                    <TabsTrigger value="assigned" className="text-xs data-[state=active]:bg-background">Assigned</TabsTrigger>
                  </TabsList>
                </Tabs>
